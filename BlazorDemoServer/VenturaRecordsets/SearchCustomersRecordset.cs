    ...
    public partial class SearchCustomersRecordset : ResultsetObservable<SearchCustomersRecordset, SearchCustomersRecord>, IRecordsetBase
    {
        ...
        public SearchCustomersRecordset()
        {
            ...
            _sqlscript = "SELECT TOP (@RowLimit) [customer_id],[first_name],[last_name],[phone],[email],[street],[city],[state],[zip_code]" + CRLF +
                         ...
                         "WHERE (@SearchText IS NULL AND @CustomerId IS NULL)" + CRLF +
                         "OR (@CustomerId IS NULL OR customer_id = @CustomerId)" + CRLF +
                         "AND (@SearchText IS NULL OR first_name LIKE '%' + @SearchText + '%' OR @SearchText IS NULL OR last_name LIKE '%' + @SearchText + '%')" + CRLF +
