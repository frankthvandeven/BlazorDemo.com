...
namespace BlazorDemo.Server.VenturaRecordsets
{
	...
	public partial class SearchOrdersRecordset : ResultsetObservable<SearchOrdersRecordset, SearchOrdersRecord>, IRecordsetBase
	{
		...
		public SearchOrdersRecordset()
		{
			...
			_sqlscript = "SELECT TOP (@RowLimit) o.order_id, o.customer_id ,o.order_status,o.order_date,o.required_date,o.shipped_date,o.store_id,o.staff_id," + CRLF +
			             "       c.first_name, c.last_name, c.city" + CRLF +
			             "FROM [sales].[orders] o LEFT JOIN sales.customers c on c.customer_id = o.customer_id" + CRLF +
			             "WHERE (@SearchText IS NULL AND @order_id IS NULL)" + CRLF +
			             "OR (@order_id IS NULL OR o.order_id = @order_id)" + CRLF +
			             "AND (@SearchText IS NULL OR c.first_name LIKE '%' + @SearchText + '%' OR c.last_name LIKE '%' + @SearchText + '%')" + CRLF +
