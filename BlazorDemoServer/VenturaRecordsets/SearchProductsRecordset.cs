...
namespace BlazorDemo.Server.VenturaRecordsets
{
	...
	/// </summary>
	public partial class SearchProductsRecordset : ResultsetObservable<SearchProductsRecordset, SearchProductsRecord>, IRecordsetBase
	{
		...
		public SearchProductsRecordset()
		{
			_resultsets = new IResultsetBase[] { this };

			_sqlscript = "SELECT TOP(@RowLimit) [product_id],[product_name],[brand_id],[category_id],[model_year],[list_price]" + CRLF +
			             "FROM [production].[products]" + CRLF +
			             "WHERE (@SearchText IS NULL AND @ProductId IS NULL)" + CRLF +
			             "OR (@ProductId IS NULL OR product_id = @ProductId)" + CRLF +
			             "AND (@SearchText IS NULL OR product_name LIKE '%' + @SearchText + '%')" + CRLF +
			             "ORDER BY product_id";
		...
		/// <summary>
		/// Database Column NotUpdateable. Table [production].[products]. NotReadonly. NotNull.
		/// </summary>
		...
		public void Append()
		{
			...
			this.InsertItem(index, new SearchProductsRecord());
			this.CurrentRecordIndex = index;
		}

		public void Append(SearchProductsRecord record)
		{
			int index = this.RecordCount;
		...
		public SearchProductsRecord NewRecord()
		{
			return new SearchProductsRecord();
		}

		protected override SearchProductsRecord InternalCreateExistingRecordObject(object[] columnvalues) => new SearchProductsRecord(columnvalues);

		byte[] IRecordsetBase.Hash
		{
			get { return new byte[] { 88, 188, 247, 235, 66, 81, 44, 98, 93, 171, 112, 41, 66, 101, 109, 58 }; }
		}

		string IRecordsetBase.HashString
		{
			get { return "58BCF7EB42512C625DAB702942656D3A"; }
		}
		...
		public void SetExecSqlParams(string SearchText, int? ProductId)
		{
			_inputparametervalues[0] = SearchText;
			_inputparametervalues[1] = ProductId;
		}

		public void ExecSql(string SearchText, int? ProductId)
		{
			_inputparametervalues[0] = SearchText;
			_inputparametervalues[1] = ProductId;
		...
		public void ExecSql(Connector connector, string SearchText, int? ProductId)
		{
			_inputparametervalues[0] = SearchText;
			_inputparametervalues[1] = ProductId;
		...
		public async Task ExecSqlAsync(string SearchText, int? ProductId)
		{
			_inputparametervalues[0] = SearchText;
			_inputparametervalues[1] = ProductId;
		...
		public async Task ExecSqlAsync(Connector connector, string SearchText, int? ProductId)
		{
			_inputparametervalues[0] = SearchText;
			_inputparametervalues[1] = ProductId;
	...
	public sealed partial class SearchProductsRecord : IRecordBase, INotifyPropertyChanged
	{
		...
		public SearchProductsRecord(object[] columnvalues)
		{
...
namespace BlazorDemo.Server.VenturaRecordsets
{
	public partial class SearchProductsRecord
	{
		partial void OnAfterPropertyChanged(string propertyName)
		{
